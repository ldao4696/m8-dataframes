...
pop.name <- names.2013[max(prop.2013) == prop.2013]
...
MostPopular <- function(my.year){
  ...
  most.pop <- names.year[max(prop.year) == prop.year]
  return(paste(most.pop, "was the most popular female name in", my.year))
}


# What was the most popular female name in 1994?
most.pop.name <- MostPopular(1994)
